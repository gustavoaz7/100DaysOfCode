	/*
	Coding in function ```sortIt```. function accept 1  parameters ```arr```, it's a number array. Your task is to sort the array according to the specified 
...
*/
  ...
  for (var i=0; i<arr.length; i++) {
  	same[arr[i]]=same[arr[i]] ? same[arr[i]]+1 : 1;
  }
  pos.sort((a,b) => {
	  if (same[a]===same[b]) { 
	  return b - a;}
	  else { return same[a] - same[b];
  	}
  });
