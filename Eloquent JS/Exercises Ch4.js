...
function range(start, end, step) {
 	let arr = [], i;
  	step = step || 1;
  	if (step > 0) {
      for (i = start; i <= (end-start+1); i += step) {
        arr.push(i); 
      }
      return arr;
    }
  	for (i = start; i >= (start-end-1); i += step) {
      arr.push(i); 
    }
    return arr;
}
...
Write a function arrayToList that builds up a data structure like the previous one when given [1, 2, 3] as argument, and 
write a listToArray function that produces an array from a list. Also write the helper functions prepend, which takes an element and 
a list and creates a new list that adds the element to the front of the input list, and nth, which takes a list and a number and 
...
function nth(list, n) {
 	if (!list || n < 0) return;
  	return !n ? list.value : nth(list.rest, n-1)
}

console.log(arrayToList([10, 20]));  // → {value: 10, rest: {value: 20, rest: null}}
console.log(listToArray(arrayToList([10, 20])));  // → [10, 20, 30]
